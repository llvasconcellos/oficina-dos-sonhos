...
class JCEInstallerPlugin extends JObject
{
	...
	function install()
	{
		...
		if (!empty ($pname)) {
			...
			$this->parent->setPath('extension_root', JCE_PLUGINS .DS. $pname);
		} else {
		...
		// ^ Altered db query for #__jce_plugins
		$query = 'SELECT `id`' .
				' FROM `#__jce_plugins`' .
		...
		if ($id) {
			...
			$elements 	= $this->manifest->getElementByPath('elements');			
			...
			//$row->variables 	= $this->parent->getVariables();
		 ...
		 */
		 // Add to Default Group
			if( $row->type == 'plugin' ){				
				...
				$grow->load( $gid );				
				// Add to plugins list
				$grow->plugins 	= $grow->plugins . ',' . $row->id;
				// Add to last row if plugin has a layout icon
				if( $row->layout ){
					$grow->rows		= $grow->rows . ',' . $row->id;
				}				
				if (!$grow->store()) {
	...
	function uninstall($id, $clientId )
	{
		...
		$this->parent->setPath('extension_root', JCE_PLUGINS . DS . $row->name);
