function PanoramioLayerCallback(map, json, panoLayer) {
  for (var i = 0; i < json.photos.length; i++) {
    var photo = json.photos[i];
    if (!panoLayer.ids[photo.photo_id]) {
      var marker = this.createMarker(map, photo, panoLayer.markerIcon);
...
PanoramioLayerCallback.prototype.createMarker = function(map, photo, baseIcon) {
  ...
  GEvent.addListener(marker, "click", function() {
    map.openInfoWindow(marker.getLatLng(), marker.html);
  });
...
function PanoramioLayer(map, opt_opts) {
  var me = this;
  me.ids = {};
  me.mgr = new GMarkerManager(map, {maxZoom: 19});
  ...
  GEvent.addListener(map, "moveend", function() {
    if (me.enabled) {
      ...
      me.load(map, me, {maxy: northEast.lat(), miny: southWest.lat(), maxx: northEast.lng(), minx: southWest.lng()});
    }
  });
}

PanoramioLayer.prototype.enable = function(map) {
  this.enabled = true;
...
PanoramioLayer.prototype.load = function(map, panoLayer, userOptions) {
  var options = {
    order: "popularity",
    set: "public",
    from: "0",
  ...
  eval(callbackName + " = function(json) { var pa = new PanoramioLayerCallback(map, json, panoLayer);}");
