<?php
/**
* @version		$Id: manager.php 46 2009-05-26 16:59:42Z happynoodleboy $
* @package      JCE
	...
	*/
	function getRootDir()
	{
		...
		if ($this->isSuperAdmin()) {
			...
			$base = preg_replace(array('/\$id/', '/\$username/', '/\$usertype/', '/\$group/'), array($user->id, strtolower($user->username), strtolower($user->usertype), strtolower($this->_group->name)), $base);
		}			
		// Clean
	...
	*/
	...
	{									
		...
		if ($folders) {
			$result .= '<ul>';
			foreach ($folders as $folder) {
				$result .= '<li id="'. $this->escape($folder['url']) .'"><div class="tree-row"><div class="tree-image"></div><span class="folder"><a href="javascript:;">'. $folder['name'] .'</a></span></div>';
