...
$faq[] = array("왜 자동으로 로그아웃되나요?", "로그인할 때에 <i>자동 로그인</i> 박스에 체크를 하지 않으면 일정시간후 게시판은 사용자를 로그 아웃합니다. 이것은 다른 사람이 계정을 남용하는 것을 막아줍니다. 계속 로그인 상태로 있으려면 로그인 할 때 박스에 체크를 하십시오, 도서관이나 인터넷 까페 같이 여러사람이 컴퓨터를 공유하는 곳에서는 사용하지 않는 것이 좋습니다.");
...
$faq[] = array("--","개인 정보");
$faq[] = array("개인 정보는 어떻게 바꾸나요?", "모든 정보(등록한)는 데이터베이스에 저장되어 있습니다. 수정하려면  <u>개인 정보</u> 링크(일반적으로 화면 상단에 있음)를 클릭하십시오. 여기에서 사용자 정보를 변경할 수 있습니다");
...
$faq[] = array("내가 사용하는 언어가 리스트에 없습니다!", "관리자가 해당 언어를 설치하지 않았거나 게시판을 해당 언어로 번역한 것이 없을 경우입니다. 게시판 관리자에게 필요한 언어의 설치를 요구하거나, 만약 번역된 것이 없다면 번역을 한번 시도해 보십시오. 보다 자세한 사항은 phpBB 그룹 웹사이트를 참조하십시오(see link at bottom of pages)");
...
$faq[] = array("이메일 링크를 클릭하니까 로그인하라고 하는데요?", "등록된 사용자만이 내장된 이메일 기능을 사용하여 이메일을 보낼 수 있습니다(운영자가 이 기능을 살려 놓은 경우에만). 이것은 익명의 사용자가 이메일 시스템을 남용하는 것을 막고자 함입니다.");
...
$faq[] = array("사용자 그룹 관리자는 어떻게 되나요?", "사용자 그룹은 게시판 관리자에 의해서 만들어지고 게시판 관리자로 지정됩니다. 사용자 그룹을 만들고 싶으면 우선 운영자를 접촉하십시오.그런 다음 그들에게 비공개 메시지를 보내십시오.");
