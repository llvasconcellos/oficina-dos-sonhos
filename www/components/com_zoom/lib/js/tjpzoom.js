...
function zoom_set(evt) {
 ...
 if(zoomid=='' || parseInt($(zoomid+'_container').style.width) == 0) {return true;}
...
function zoom_init() {
 $(zoomid+'_clip').style.width=objw*zoomamount+'px';
 $(zoomid+'_clip').style.height=objh*zoomamount+'px';
 try {
    $(zoomid + '_trigger').focus();
 } catch(e) {}
}
...
function zoom_move(evt) {
 if(typeof(evt) == 'object') {
  ...
  if(evt.pageX) {
   ...
   if(typeof($(zoomid)+1) == 'number') {return true;} //mert az ie egy ocska kurva
   xx=evt.x - ieox;
   yy=evt.y - ieoy;
  }
 } else {
 ...
 lastxx=xx; lastyy=yy;
 $(zoomid+'_clip').style.margin=((yy-zoomh/2 > 0)?(zoomh/2-yy*zoomamount):(yy*(1-zoomamount)))+'px 0px 0px '+((xx-zoomw/2 > 0)?(zoomw/2-xx*zoomamount):(xx*(1-zoomamount)))+'px';
 $(zoomid+'_container').style.margin=((yy-zoomh/2 > 0)?(yy-zoomh/2):(0))+'px 0px 0px '+((xx-zoomw/2 > 0)?(xx-zoomw/2):(0))+'px';

 w2=((xx+zoomw/2<objw)?((zoomw/2<xx)?(zoomw):(zoomw/2+xx)):(zoomw/2+objw-xx)); if(w2<0) {w2=0;} $(zoomid+'_container').style.width=w2+'px';
 h2=((yy+zoomh/2<objh)?((zoomh/2<yy)?(zoomh):(zoomh/2+yy)):(zoomh/2+objh-yy)); if(h2<0) {h2=0;} $(zoomid+'_container').style.height=h2+'px';
 return false;
}
