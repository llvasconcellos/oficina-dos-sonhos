<?php
/**
* @version		updater.php 07-04-2009
...
class JCEUpdater extends JObject {
	...
	*/
	...
	function checkTable( $table ){
		...
		$query = 'SELECT COUNT(*) FROM #__jce_'. $table;
		$db->setQuery( $query );
		return $db->query();
	}
	function purgeDB(){
	...
	function createPluginsTable(){
		...
		if( !$db->query() ){
			$mainframe->enqueueMessage( JText::_('CREATE TABLE PLUGINS ERROR'), 'error' );
			return false;
		}else{
			return true;
		}
	...
	function createExtensionsTable(){
		...
		if( !$db->query() ){
			$mainframe->enqueueMessage( JText::_('CREATE TABLE EXTENSIONS ERROR'), 'error' );
			return false;
		}else{
			return true;
		}
	...
	function createGroupsTable(){
		...
		if( !$db->query() ){
			$mainframe->enqueueMessage( JText::_('CREATE TABLE GROUPS ERROR'), 'error' );
			return false;
		}else{
			return true;
		}
	...
	function installEditor(){
		...
		if( file_exists( $path .DS. 'jce.php' ) && file_exists( $path .DS. 'jce.xml' ) ){	
			...
			$mainframe->redirect( 'index.php', JText::_('EDITOR FILES MISSING') );
			return false;
		}
		$mainframe->redirect( 'index.php?option=com_jce', JText::_('EDITOR INSTALL SUCCESS') );	
	}	
}
?>