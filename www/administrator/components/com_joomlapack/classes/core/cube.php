...
jpimport('classes.core.domain.installer');
...
class JoomlapackCUBE extends JoomlapackObject {
	...
	/**
	 * Relative name of the backup archive
	 * @var string
	 */
	var $relativeBackupFilename = '';
	...
	function JoomlapackCUBE( $OnlyDBMode = false )
	{
		...
		if ($this->_OnlyDBMode) {
			...
			$this->relativeBackupFilename = JoomlapackAbstraction::getExpandedTarName( $this->_archiveExtension, true );
			$archiveFile = JoomlapackAbstraction::TranslateWinPath($configuration->get('OutputDirectory').DS.$this->relativeBackupFilename);
			JoomlapackLogger::WriteLog(_JP_LOG_DEBUG, "Expanded archive file name: " . $archiveFile);
			
			JoomlapackLogger::WriteLog(_JP_LOG_DEBUG, "Initializing backup archive");
	...
	function _selectAlgorithm(){
		...
		switch( $this->_currentDomain )
		{
			...
			case "finale":
				return "(null)";
				break;
			case "installer": // Installer deployment fixed on smart algorithm
				return $this->_OnlyDBMode ? "(null)" : "smart";
				break;
			case "PackDB":
	...
	function _getNextObject(){
		...
		switch( $this->_currentDomain )
		{
			case "init":
				...
				$this->_currentObject = new JoomlapackDomainInstaller();
				$this->_currentDomain = "installer";
				return 0;
				break;
			case "installer":
