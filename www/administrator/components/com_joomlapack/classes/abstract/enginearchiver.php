...
{
	define('JPPACK_CHUNK', $registry->get('mnArchiverChunk', 0));
}
else
{
	define('JPPACK_CHUNK', 524288);
}
 ...
 * @abstract 
 */
class JoomlapackCUBEArchiver extends JObject
{
	/**
	 * JPA transformation source archive absolute file name
	 *
	 * @var integer
	 */
	var $_xform_source;
	...
	/**
	 * The last time we used a keep-alive MySQL dummy write operation
	 *
	 * @var float
	 */
	var $startTime = 0;
	...
	/**
	 * The archive's comment. It's currently used ONLY in the ZIP file format
	 * @var string
	 */
	var $_comment;
	...
	function addFileList( &$fileList, $removePath = '', $addPath = '' )
	{
		if( !is_array($fileList) ) {
			JoomlapackLogger::WriteLog(_JP_LOG_WARNING, __CLASS__ . " :: ".JText::_('CUBE_ARCHIVER_WARN_EMPTYFL'));
			return false;
		}
		...
		foreach( $fileList as $file ) {
			...
			if( $ret === false ) {
				JoomlapackLogger::WriteLog(_JP_LOG_WARNING, JText::sprintf('CUBE_WARN_UNREADABLEFILE', $file));
			}
		}
	...
	{
		// Initialize the data for installer deployment
		$this->_xform_source = $sourceJPAPath;
	}
	...
	*/
	function _addRemovePaths( $p_filename, $p_remove_dir, $p_add_dir ) {
		...
		$p_remove_dir = ($p_remove_dir == '') ? '' : JoomlapackHelperUtils::TranslateWinPath( $p_remove_dir ); //should fix corrupt backups, fix by nicholas
