 ...
 * @link http://www.matware.com.ar/joomla/mtwmigrator-english.html
 * @license		GNU/GPL
 */
...
class mtwMigratorModelMigrate extends JModel
{
	...
	function __construct() {
		...
		//$this->_status = array();
/*
		if( $option['host'] == '' || $option['user'] == '' || $option['password'] == '' || $option['database'] == '' || $option['prefix'] == '' ) {
			echo "-1";
			$this->setError('Please complete the Global Configuration');
			return false;
		}
	}
 */
	...
	function &getConfig() {
        ...
        $this->_option['driver']   = 'mysql';
		$this->_option['host']     = $mtwCFG['hostname'];
        $this->_option['user']     = $mtwCFG['username']; 
        $this->_option['password'] = $mtwCFG['password'];
        $this->_option['database'] = $mtwCFG['dbname'];  
        $this->_option['prefix']   = $mtwCFG['prefix']; 
	...
	function &getConnectToExternalDB () {
		...
		$this->_externalDB = & JDatabase::getInstance( $this->_option );
 ...
 */
	...
	function __startBackup() {
        ...
        $backupFile = "components/com_mtwmigrator/backups/" . $this->_option['database'] . "-" . date("YmdHis") . '.gz';
        $command = "mysqldump --opt -h " . $this->_option['host'] . " -u " . $this->_option['user'] . " -p" .  $this->_option['password'] . " " . $this->_option['database'] . "| gzip > " . $backupFile;   
		//echo $command . "<br><br>";
        system($command);
    ...
    function __migrateGroups() {
		...
		$query = "DROP TABLE " . $this->_option['prefix'] . "core_acl_aro_groups";
		...
		$query = "SELECT `group_id` AS `id` , `parent_id`, `name`, `lft`, `rgt`, `name` AS `value`"
				." FROM " . $this->_option['prefix'] . "core_acl_aro_groups"
				." WHERE `id` > 2";
		...
		$query = "SELECT `id`, `name`"
				." FROM " . $this->_option['prefix'] . "groups"
				." WHERE `id` > 2";
    ...
    function __migrateSections() {
		...
		$query = "SELECT `id`, `title`, `name`, NULL, `image`, `scope`, `image_position`,"
			    ...
			    ." FROM " . $this->_option['prefix'] . "sections";
    ...
    function __migrateCategories() {
        ...
        $query = "SELECT `id`, `parent_id`, `title`, `name`, NULL, `image`, `section`, `image_position`, `description`, `published`, `checked_out`, `checked_out_time`, `editor`, `ordering`, `access`, `count`, `params`"
                ." FROM " . $this->_option['prefix'] . "categories";
    ...
    function __migrateContent() {
		...
		$query = "SELECT `id`, `title`, NULL AS `alias`, `title_alias`, `introtext`, `fulltext`, `state`, `sectionid`, `mask`, `catid`, `created`, `created_by`, `created_by_alias`, `modified`, `modified_by`, `checked_out`, `checked_out_time`, `publish_up`, `publish_down`, `images`, `urls`, `attribs`, `version`, `parentid`, `ordering`, `metakey`, `metadesc`, `access`, `hits`, NULL"
				." FROM " . $this->_option['prefix'] . "content"
				." ORDER BY id ASC";
		...
		for ($i=0; $i<$count; $i++) {
			...
			if ($data[$i]->images) {
				...
				for ($y=0; $y<$count_images; $y++) {
						...
						//echo "> " . strpos($data[$i]->introtext, "{mosimage}") . " < == ";

						$intro = strpos($data[$i]->introtext, "{mosimage}");

						// BUG: if {mosimage} is in 0 position intro replace not work - this bug is fixed?
						// Patch from Mark Berbiers (mberbiers) 
						if ($intro || $intro == 0) {
								$data[$i]->introtext = str_replace("{mosimage}", $img , $data[$i]->introtext );
								echo "I";
						}else{
								$data[$i]->fulltext = str_replace("{mosimage}", $img , $data[$i]->fulltext );
								echo "F";
						}
						$intro = 0;
				}
    ...
    function __migrateContentFrontpage() {
		...
		$query = "SELECT `content_id`, `ordering` FROM " . $this->_option['prefix'] . "content_frontpage";
    ...
    function __migrateModulesMenus() {
        ...
        $query = "SELECT `id`, `title`, `content`, `ordering`, `position`, `checked_out`,"
				...
				." FROM " . $this->_option['prefix'] . "modules"
        ...
        $query = "SELECT moduleid, menuid FROM " . $this->_option['prefix'] . "modules_menu";
        $this->_externalDB->setQuery( $query );
    ...
    function __migratePolls() {
        ...
        $query = "SELECT `id`, `title`, NULL, `voters`, `checked_out`,"
                ...
                ." FROM " . $this->_option['prefix'] . "polls";
        ...
        $query = "SELECT `id`, `pollid`, `text`, `hits`"
                ." FROM " . $this->_option['prefix'] . "poll_data";
        ...
        $query = "SELECT `id`, `date`, `vote_id`, `poll_id`"
                ." FROM " . $this->_option['prefix'] . "poll_date";
        ...
        $query = "SELECT `pollid`, `menuid`"
                ." FROM " . $this->_option['prefix'] . "poll_menu";
    ...
    function __migrateWeblinks() {
		...
		$query = "SELECT `id`, `catid`, `sid`, `title`,  NULL AS `alias`, `url`, `description`, `date`, `hits`, `published`, `checked_out`, `checked_out_time`, `ordering`, `archived`, `approved`, `params`"
				." FROM " . $this->_option['prefix'] . "weblinks";
    ...
    function __migrateContacts() {
        ...
        $query = "SELECT `id`, `name`, NULL AS `alias`, `con_position`, `address`, `suburb`, `state`, `country`, `postcode`, `telephone`, `fax`, `misc`, `image`, `imagepos`"
                ...
                ." FROM " . $this->_option['prefix'] . "contact_details";
