...
if ($this->require_ftp) {
	...
	<input type="hidden" name="return-url" value="<?php echo base64_encode('index.php?option=com_phocagallery&view=phocagalleryi&tmpl=component&tab='.$this->tmpl['currenttab']['flashupload']); ?>" />
	<?php /* !!!!!! $refreshSite for flash upload is set in view.html.php */ ?>
</form>
<?php
echo PhocaGalleryHelperUpload::renderCreateFolder($this->session->getName(), $this->session->getId(), $currentFolder, 'phocagalleryi' );
?>
</div>